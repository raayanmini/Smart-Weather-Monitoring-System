...
static void ST7735_WriteChar_Bg(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) {
    uint32_t i, b, j;
    // Calculate the mask based on font storage (assuming font data is MSB-first)
    uint16_t bit_mask = 1 << (font.width - 1); // For example, 0x80 for 8-bit width
    ...
    for(i = 0; i < font.height; i++) {
        ...
        for(j = 0; j < font.width; j++) {
            // Shift the relevant bit to the MSB position and check
            if((b << j) & (bit_mask << (16 - font.width))) {
                uint8_t data[] = { color >> 8, color & 0xFF };
                ST7735_WriteData(data, sizeof(data));
            } else {
...
void ST7735_WriteString(uint16_t x, uint16_t y, const char* str, FontDef font, uint16_t color,uint16_t bgcolor) {
    ...
    while(*str) {
        ...
        if(bgcolor == DISABLE)
        	...
        	ST7735_WriteChar_Bg(x, y, *str, font, color,bgcolor);
